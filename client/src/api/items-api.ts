...
// Search Item
export async function searchItem(searchText: string, idToken: string): Promise<Item[]> {
...
export async function patchItem(
  idToken: string,
  itemId: string,
  updatedRequest: UpdateItemRequest
): Promise<void> {
  await Axios.patch(`${apiEndpoint}/items/${itemId}`, JSON.stringify(updatedRequest), {
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${idToken}`
    }
